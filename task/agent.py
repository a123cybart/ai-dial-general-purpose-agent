...
from aidial_sdk.chat_completion import Choice, Message, Request, Response, Role
...
class GeneralPurposeAgent:
    ...
    def _prepare_messages(self, messages: list[Message]) -> list[dict[str, Any]]:
        ...
        unpacked_messages = unpack_messages(
            messages, self.state.get(TOOL_CALL_HISTORY_KEY, [])
        )
    ...
    async def _process_tool_call(
        ...
        stage = StageProcessor.open_stage(choice=choice, name=f"Tool: {tool_name}")
        ...
        if tool:
            ...
            stage.append_content(tool_response.content)
        else:
            stage.append_content(f"⚠️ Tool '{tool_name}' not found.")
            return Message(
                role=Role.TOOL,
                name=tool_name,
                content=f"⚠️ Tool '{tool_name}' not found.",
